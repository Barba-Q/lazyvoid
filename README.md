...
It creates fully automated btrfs snapshots - as long as btrfs is utilized - right before every system update.
...
If you want a fresh, out-of-the-box experience with KDE Plasma, Wayland, grab the latest bootable ISO https://drive.google.com/drive/folders/1G3_DssizTY0t6ZxRBynfO3PLemHSnY02?usp=sharing, boot it, install it, firstboot will handle everything else, done.  It's set and forget.
...
This is exactly where Void Linux changes the equation. Unlike bleeding-edge distros that use their users as beta testers, Void is a pragmatic rolling release. They strictly push stable packages to their repositories. This fundamentally reduces the risk of system breakages right out of the gate, making automated background updates actually viable.

Lazyvoid embraces this reality further and builds a safety net around it. 
...
## Tested 
Lazyvoid isn't just some weekend experiment. These core scripts have been continuously running, evolving, and actively monitored on multiple different hardware setups for over two years now.

We've thrown everything at it – including booting up test machines that had been collecting dust offline for months, just to see if the background update mechanism survives the massive package backlog. It did. Over time, the scripts have been heavily refined, stripped of unnecessary bloat, and expanded with bulletproof features. What you download is a mature, battle-hardened setup that simply works and refuses to break. It's completely transparent and hackable if you dare to.

## Up to date
Lazyvoid updates itself. Every time the background service runs, it syncs directly with this master branch on GitHub. It pulls the raw scripts, compares them to your local files, and patches itself if there's a new version.

Why does this matter? Because Void Linux is a moving target. If it drops a core system update tomorrow that requires a tweak in our automation , I just push a fix to GitHub and your system will automatically pull the updated logic. We don't need (and want) to maintain a 'real' custom package repository, deal with GPG keys, or compile custom binaries just to update a few kilobytes of shell scripts. It’s direct, it’s bulletproof, and it keeps our footprint at absolute zero.
